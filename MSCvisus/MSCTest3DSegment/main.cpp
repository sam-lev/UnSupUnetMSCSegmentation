...
int main(int argc, char** argv) {
	...
	CrystalExtractingMSC<float>* msc = 
		new CrystalExtractingMSC<float>(bgf,bmsh,bmf); 
	msc->ComputeFromGrad();
	...
	// if user supplied persistence then use that else use the 10 percent
	if (argc > 6) tenpercent = cancelto;
	msc->ComputeHeirarchy(tenpercent);

	// now set the selection persistence 
	msc->setAbsolutePersistence(tenpercent  );
	

	// now create the crystal segmentation!
	msc->RegisterCells();

	// now it should all be available!

	// test things out by dumping one crystal as .xyz


	bool onedcell = false;
	for (map<CrystalExtractingMSC<float>::CPair, CrystalExtractingMSC<float>::DCell*>::iterator it = 
		msc->mCellRegistry.begin(); it != msc->mCellRegistry.end(); it++) {
			// the first will be a pair of cell indexes of the critical cells that generate the dcell - 
			// for sanity, lets check that this is true!
			CrystalExtractingMSC<float>::CPair p = (*it).first;
			CrystalExtractingMSC<float>::DCell* d = (*it).second;

			//sanity check
			if (d->lower != p.first || d->upper != p.second) {
				printf("ERROR: cell id mismatch: %d-Dcell[%d] expecting endpoints %d-%d, got %d-%d\n",
					d->D, d, d->lower, d->upper, p.first, p.second);
			}
			char OUTNAME[1024];
			sprintf(OUTNAME, "out/d_%d_%d-%d_%d.xyz", d->D, 
				msc->nodes[d->lower]->index, msc->nodes[d->upper]->index, d);
			FILE* fout = fopen(OUTNAME, "w");

			if (! onedcell) {

				printf("\n");
				printf("Cell %d, dim %d, lower[%d]=%d, upper[%d]=%d\n", d, d->D,
					d->lower, msc->nodes[d->lower]->index, 
					d->upper, msc->nodes[d->upper]->index);
				
				// an example of how to query the mesh structure to get the point locations of a 
				// cell index. this works for the cell indexes of the critical points as well as
				// the cell indexes of the geometry CellGEometry()
				float cL[3]; float cU[3];
				bmsh->centroid(d->lower, cL);
				bmsh->centroid(d->upper, cU);
				printf("----(%.2f,%.2f,%.2f)\n----(%.2f,%.2f,%.2f)\n", 
					cL[0], cL[1], cL[2], cU[0], cU[1], cU[2]); 

				// now lets iterate through the geometry of a cell, but only count things
				// yes, this could use the size() function, but the example really is to 
				// show the iteration.
				set<CELL_INDEX_TYPE> geometry;
				msc->CellGeometry(d, geometry);
				printf("geom size = %d\n", geometry.size());
				for (set<CELL_INDEX_TYPE>::iterator sit = geometry.begin(); 
					sit != geometry.end(); sit++) {
						float cc[3];
						bmsh->centroid(*sit, cc);
						fprintf(fout, "%.2f %.2f %.2f\n", cc[0], cc[1], cc[2]);
				}



				// now lets look at getting the boundary of a cell
				// the facets and cofacets
				printf("FACETS/COFACETS: endpoints are n[id]=(dim,value)\n");
				// the neighbors of the cell
				for (set<CrystalExtractingMSC<float>::DCell*>::iterator it2 = d->neighbors.begin(); 
					it2 != d->neighbors.end(); it2++) {
						CrystalExtractingMSC<float>::DCell* n = *it2;
						printf("\t%d:n[%d]=(%d,%f)-n[%d]=(%d,%f)\n", 
							n->D, 
							n->lower, msc->nodes[n->lower]->index, msc->nodes[n->lower]->value,
							n->upper, msc->nodes[n->upper]->index, msc->nodes[n->upper]->value);
				}
			...
			FILE* foo = fopen("arcx.txt", "w");

			for (int i =0; i < msc->arcs.size(); i++) {
				vector<CELL_INDEX_TYPE> res;
				msc->fillGeometry(msc->arcs[i], res);
				fprintf(foo, "%d %d %d\n", msc->arcs[i]->lower->cellid, msc->arcs[i]->upper->cellid, res.size());
			}
			fclose(foo);
