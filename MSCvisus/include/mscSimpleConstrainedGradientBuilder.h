#ifndef MSC_SIMPLE_CONSTRAINED_GRADIENT_BUILDER
#define MSC_SIMPLE_CONSTRAINED_GRADIENT_BUILDER
...
class mscSimpleConstrainedGradientBuilder {
protected:
	...
	virtual void init_asc() {
		...
		for (one_cells.begin(it); one_cells.valid(it); one_cells.advance(it)) {
			...
			// look up value
			int equals = (mClassifier->getId(vertex1) != mClassifier->getId(vertex2)? 1 : 0);
		...
		for (int i = 2; i <= my_mesh_handler->max_dim(); i++) {
			printf("seg stuff %d\n", i);
			cellIterator it2;
			...
			for (d_cells.begin(it2); d_cells.valid(it2); d_cells.advance(it2)) {
				...
				int value = 0;
				for (facets.begin(fit); facets.valid(fit); facets.advance(fit)) {
					CELL_INDEX_TYPE facet = facets.value(fit);
					if (my_grad_field->get_dim_asc_man(facet) != 0) {
						value = 1;
						break;
					}
				}
				my_grad_field->set_dim_asc_man(cellid, value);
				//printf("asdfasdl;fkjas;dklfj %d\n", cellid);
			}
		}
	}
	...
	virtual void init_dsc() {
		...
		for (n_cells.begin(it); n_cells.valid(it); n_cells.advance(it)) {
			// the id of the n-1-cell
			CELL_INDEX_TYPE cellid = n_cells.value(it);
			
			// all boundary is "boundary"
			if (my_mesh_handler->boundary_value(cellid) > 0) {
				set_cell_dsc_count(cellid, 1);
				continue;
			}
			// the "Voxels" of the n-1-cell

			cellIterator cfit;
			iteratorOperator& cofacets = my_mesh_handler->cofacets(cellid, cfit);
			cofacets.begin(cfit);	//set to first vertex
			CELL_INDEX_TYPE voxel1 = cofacets.value(cfit);
			cofacets.advance(cfit);  // next vertex
			CELL_INDEX_TYPE voxel2 = cofacets.value(cfit);

			// look up value
			int equals = (mClassifier->getId(voxel1) != mClassifier->getId(voxel2)? 1 : 0);
			set_cell_dsc_count(cellid, equals);
		}
...
public:
	...
	mscSimpleConstrainedGradientBuilder(
