...
#define MAX_CELLID_VALUE ((size_t) -1)

using namespace std;

// standins for the containers that will house the node and arc data
...
class ManifoldManager {
public:
	mscTopoArray<CELL_INDEX_TYPE> created;
	mscTopoArray<CELL_INDEX_TYPE> destroyed;
	mscTopoArray<CELL_INDEX_TYPE> basenode; // node id
...
public:
	...
	void setFirstNonzeroPersistence(float perc) {
		int pos = 1;
		while (cancel_history[pos]->persistence <= 0 looky && pos < cancel_history.size()) pos++;
	...
	void ComputeFromGrad() {
		...
		// then add arcs
		for (map<CELL_INDEX_TYPE, node<dtype>*>::iterator nit = nodes.begin();
			nit != nodes.end(); nit++) {
	...
	void ValidateComplex() {
		map<CELL_INDEX_TYPE, node<float>*>::iterator nit = nodes.begin();
