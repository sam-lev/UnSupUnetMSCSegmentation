...
int main(int argc, char** argv) {
	...
	if (argc < 5) {
		printf("Usage: test filename x y computemode\n\
			   			   \t 0 = randomized\n\
						   			   \t 1 = greedy\n\
	...
	if (!test_data->load_data(filename, X*Y*1, &a)) printf("ERROR: DID NOT LOAD DATA\n");
	...
	if (gradienttype == 0) {
		...
		printf("using convergent gradient - accurate basins\n");
	...
	TalassMSC<float>* msc = new TalassMSC<float>(bgf, bmsh, bmf);
	msc->ComputeFromGrad();
	msc->ComputeHeirarchy();



	msc->WriteCancelHistory(filename);
	...
	msc->setFirstNonzeroPersistence(0);
