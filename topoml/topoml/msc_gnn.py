...
class msc_gnn:
    ...
    def select_msc(self, train_or_test='', group_name='unnamed', bounds = [], write_json = False, write_msc = False, load_msc = 'test_ridge_arcs', cvt=tuple(), val_count=10):
        ...
        G, node_id, node_classes, node_features = tracer.create_graphsage_input(self.positive_arcs, self.negative_arcs, gt_in_arcs=gt_in_arcs, gt_out_arcs = gt_out_arcs, val_count=val_count, val_in_arcs = val_in_arcs, val_out_arcs=val_out_arcs)
    ...
    def train(self, graph = None,
        ...
        if self.G and not train_prefix and not cvt_train:
            ...
            G,feats,id_map, walks, class_map, number_negative_samples, number_positive_samples = format_data(dual=self.G, features=self.features, node_id=self.node_id, id_map=self.node_id, node_classes=self.node_classes, train_or_test = '', scheme_required = True, load_walks=load_walks)
